...
abstract class DigdagBlockScalarImpl(node: ASTNode) : DigdagScalarImpl(node) {
    companion object {
    ...
    override fun getContentRanges(): List<TextRange> = CachedValuesManager.getCachedValue(this) {
        ...
        val contentRanges = linesNodes.asSequence().mapNotNull { line ->
            ...
            val start = (first.textRange.startOffset - myStart
                    + if (first.elementType == DigdagTokenTypes.INDENT) min(first.textLength, indent) else 0)
    ...
    fun locateIndent(): Int = reuseFragmentEditorIndent(this, fun(): Int {
        ...
        if (firstLine != null) {
            return DigdagUtil.getIndentInThisLine(firstLine.psi)
        } else {
            val line = linesNodes.getOrNull(1)
            if (line != null) {
                val lineIndentElement =
                    ContainerUtil.find(line) { l: ASTNode -> l.elementType == DigdagTokenTypes.INDENT }
    ...
    override fun getEncodeReplacements(input: CharSequence): List<Pair<TextRange, String>> {
        ...
        return splitLineRanges(input).zipWithNext { a, b ->
            Pair.create(
                TextRange.create(a.endOffset, b.startOffset),
                indentString
            )
        }.toList()
    }
    ...
    protected val linesNodes: List<List<ASTNode>>
        get() {
            ...
            var child = this.getFirstContentNode()
            while (child != null) {
...
fun isEol(node: ASTNode?): Boolean = when (node) {
    null -> false
    else -> DigdagElementTypes.EOL_ELEMENTS.contains(node.elementType)
}
